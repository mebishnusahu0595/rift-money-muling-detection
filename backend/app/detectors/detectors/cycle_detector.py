...
_MAX_CYCLES = 2_000  # safety cap to avoid combinatorial explosion
...
def detect_cycles(
    ...
    Optimized: vectorized edge_data building, early termination.
    """
    ...
    # Build edge lookup using vectorized groupby (much faster than iterrows)
    edge_data: Dict[Tuple[str, str], List[Tuple[float, pd.Timestamp]]] = {}
    grouped = df.groupby(["sender", "receiver"])
    for (sender, receiver), grp in grouped:
        key = (str(sender), str(receiver))
        edge_data[key] = list(zip(
            grp["amount"].astype(float).tolist(),
            pd.to_datetime(grp["timestamp"]).tolist()
        ))
